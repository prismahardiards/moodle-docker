...
WORKDIR /tmp
RUN wget https://netix.dl.sourceforge.net/project/maxima/Maxima-source/5.47.0-source/maxima-5.47.0.tar.gz \
    && tar xvf maxima-5.47.0.tar.gz \
    && cd maxima-5.47.0 \
    && ./configure --prefix=/usr --with-sbcl \
    && make -j$(nproc) \
    && make install \
    && cd / \
    && rm -rf /tmp/maxima-5.47.0*